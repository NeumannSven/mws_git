...
public:
     ...
     * @param ui32Channel GSP_SCOPE_MODULE, GSP_SCOPE_SYNC1, GSP_SCOPE_SYNC2
     *
     * @throws a MIOexception when writing to card fails,
     * or a MIOexception if pointer to card is invalid
     */
    VOID exportScope(CHAR8* pch8Filename, UINT32 ui32Timeout, UINT32 ui32Channel);
     ...
     * or a MIOexception if pointer to card is invalid
     */
    VOID exportScopeNB(CHAR8* pch8Filename, UINT32 ui32Timeout, UINT32 ui32Channel);
     ...
     * or a MIOexception if pointer to card is invalid
     */
    MIO_PROGRESS_R* getExportScopeProgress(UINT32 ui32Channel);
     ...
     *
     * @param ui32Channel - selected Channel (10 or 11)
     * @param bool8Value - FALSE = OFF, TRUE = ON
     *
     * @throws a MIOexception when writing to card fails,
     * or a MIOexception if pointer to card is invalid
     */
    VOID forceRelay(UINT32 ui32Channel, BOOL8 bool8Value);
     ...
     * or a MIOexception if pointer to card is invalid
     */
    VOID resetRelay(UINT32 ui32Channel);

    /**
     * @brief Start Scope
     *
     * @param ui32Channel  GSP_SCOPE_MODULE is only supported
     *
     * @throws a MIOexception when writing to card fails,
     * or a MIOexception if pointer to card is invalid
     */
    VOID triggerScope(UINT32 ui32Channel);
     ...
     *
     * @param ui32Channel - selected Channel (0-6) 0=Module
     * @return event state
     *
     * @throws a MIOexception when reading from card fails,
     * or a MIOexception if pointer to card is invalid
     */
    EEvent_t getEvent(UINT32 ui32Channel);

    /**
     * @brief Read Harmonics
     *
     * @param id - see EHarmonicsId_t
     * @return pointer to harmonics data structure
     *
     * @throws a MIOexception when reading from card fails,
     * or a MIOexception if pointer to card is invalid
     */
    SHarmonics_t* getHarmonics(EHarmonicsId_t id);
     ...
     * or a MIOexception if pointer to card is invalid
     */
    UINT32 getPreInitErrors(UINT32 ui32Channel);
     ...
     * or a MIOexception if pointer to card is invalid
     */
    UINT32 getPreInitEvents(UINT32 ui32Channel);
     ...
     * or a MIOexception if pointer to card is invalid
     */
    VOID setScopeConfig(SScopeConfig_t *psValue);
     ...
     * or a MIOexception if pointer to card is invalid
     */
    SScopeData_t* getScopeValues(UINT32 ui32Index, UINT32 ui32Timeout, UINT32 ui32Channel);
     ...
     * or a MIOexception if pointer to card is invalid
     */
    SScopeState_t* getScopeState(UINT32 ui32Channel);
...
protected:
     ...
     * or a MIOexception if pointer to card is invalid
     */
    VOID takeScopeLock(UINT32 ui32Timeout, UINT32 ui32Channel);
     ...
     * or a MIOexception if pointer to card is invalid
     */
    VOID giveScopeLock(UINT32 ui32Channel);
